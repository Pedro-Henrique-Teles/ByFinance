            ...
            <form action="{{ route('conta.store') }}" method="POST" class="p-6">
                ...
                <div class="mb-4 flex items-center">
                    <div class="w-3/4">
                        ...
                        <select id="banco" name="banco" required
                            ...
                            @foreach ($bandeiras as $bandeira)
                                <option value="{{ $bandeira->id }}" data-image="{{ asset($bandeira->imagem) }}"
                                    {{ $bandeira->nome == 'Visa' ? 'selected' : '' }}>
                                    {{ $bandeira->nome }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="w-1/4 ml-4">
                        <img id="imagemBanco" src="{{ asset('images/visa.png') }}" alt="Imagem do Banco"
                            class="w-16 h-16 object-contain border border-gray-300 rounded-md shadow-sm">
                    </div>
                </div>

                <!-- Saldo Inicial e Tipos -->
                <div class="mb-4 flex items-center justify-between gap-4">
                    <!-- Saldo Inicial -->
                    <div class="w-1/2">
                        <label for="saldoInicial" class="block text-sm font-medium text-gray-700">Saldo Inicial</label>
                        <input type="number" placeholder="R$ 0,00" id="saldoInicial" name="saldoInicial" step="0.01"
                            required
                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    </div>
                    ...
                    <!-- Tipo de Conta -->
                    <div class="w-1/2">
                        <label for="tipo" class="block text-sm font-medium text-gray-700">Tipo de Conta</label>
